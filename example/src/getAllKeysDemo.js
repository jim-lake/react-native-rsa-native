  ...
  try {
    ...
    for (const key of validKeys) {
      ...
      if (typeof key.class !== 'string') {
        console.log(
          `Key ${key.tag}: class must be string, got ${typeof key.class}`,
        );
        return false;
      }
      if (typeof key.type !== 'string') {
        console.log(
          `Key ${key.tag}: type must be string, got ${typeof key.type}`,
        );
        return false;
      }
      if (typeof key.public !== 'string') {
        console.log(
          `Key ${key.tag}: public must be string, got ${typeof key.public}`,
        );
        return false;
      }
      if (typeof key.tag !== 'string') {
        console.log(
          `Key ${key.tag}: tag must be string, got ${typeof key.tag}`,
        );
        return false;
      }
      if (typeof key.label !== 'string') {
        console.log(
          `Key ${key.tag}: label must be string, got ${typeof key.label}`,
        );
        return false;
      }
      if (typeof key.accessControl !== 'string') {
        console.log(
          `Key ${
            key.tag
          }: accessControl must be string, got ${typeof key.accessControl}`,
        );
        return false;
      }
      ...
      if (typeof key.size !== 'number') {
        console.log(
          `Key ${key.tag}: size must be number, got ${typeof key.size}`,
        );
        return false;
      }
      ...
      if (typeof key.extractable !== 'boolean') {
        console.log(
          `Key ${
            key.tag
          }: extractable must be boolean, got ${typeof key.extractable}`,
        );
        return false;
      }
      if (typeof key.syncronizable !== 'boolean') {
        console.log(
          `Key ${
            key.tag
          }: syncronizable must be boolean, got ${typeof key.syncronizable}`,
        );
        return false;
      }
    }
